...
```{r}
model_single_layer <- keras_model_sequential() %>%
  layer_flatten(input_shape = c(img_height, img_width, 3)) %>%
  layer_dense(length(class_names), activation = 'softmax')
...
history_single_layer <- model_single_layer %>% fit(
  ...
  epochs = 20,
  steps_per_epoch = train_tumor$samples %/% batch_size,
  validation_steps = validation_tumor$samples %/% batch_size
)
```
